...
import { getDrivingRangeClubsNearMe } from "@redux/teetime/actions";
import dayjs from "dayjs";

const LENGTH = 8;

export const DrivingCourse = ({ item }) => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { drivingRangeClubsNearMe } = useSelector(
  ...
  useEffect(() => {
    dispatch(getDrivingRangeClubsNearMe());
  }, []);
  ...
  return (
    <div className="my-4">
      {drivingRangeClubsNearMe?.length > 0 && (
        <Root>
          <div
            className={`flex flex-row lg:py-8 py-6 items-center justify-between`}
          >
            <HeadingText className={`${Heading} font-semibold`}>
              {item?.attributes?.title || "Driving range course near me"}
            </HeadingText>
            {drivingRangeClubsNearMe?.length > 8 ? (
              <button
                onClick={() => {
                  setViewAll(!viewAll);
                }}
          ...
          <ul role="list" className={CoursesGrid}>
            {drivingRangeClubsNearMe
              ?.slice(
                0,
                viewAll === true ? LENGTH : drivingRangeClubsNearMe.length
              )
              .map((client, index) => {
                const ratings =
