...
import { getPuttingGreenClubsNearMe } from "@redux/teetime/actions";
import dayjs from "dayjs";

const LENGTH = 8;

export const PuttingGreenCourse = ({ item }) => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { puttingGreenClubsNearMe } = useSelector(
  ...
  useEffect(() => {
    dispatch(getPuttingGreenClubsNearMe());
  }, []);
  ...
  return (
    <div className="my-4">
      {puttingGreenClubsNearMe?.length > 0 && (
        <Root>
          <div
            className={`flex flex-row lg:py-8 py-6 items-center justify-between`}
          >
            <HeadingText className={`${Heading} font-semibold`}>
              {item?.attributes?.title || "Putting green course near me"}
            </HeadingText>
            {puttingGreenClubsNearMe?.length > 8 ? (
              <button
                onClick={() => {
                  setViewAll(!viewAll);
                }}
          ...
          <ul role="list" className={CoursesGrid}>
            {puttingGreenClubsNearMe
              ?.slice(
                0,
                viewAll === true ? LENGTH : puttingGreenClubsNearMe.length
              )
              .map((client, index) => {
                const ratings =
