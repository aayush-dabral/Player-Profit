...
import { getTeachingProClubsNearMe } from "@redux/teetime/actions";
import dayjs from "dayjs";

const LENGTH = 4;

export const TeachingProCourse = ({ item }) => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { teachingProClubsNearMe } = useSelector(
  ...
  useEffect(() => {
    dispatch(getTeachingProClubsNearMe());
  }, []);
  ...
  return (
    <div className="my-4">
      {teachingProClubsNearMe?.length > 0 && (
        <Root>
          <div
            className={`flex flex-row lg:py-8 py-6 items-center justify-between`}
          >
            <HeadingText className={`${Heading} font-semibold`}>
              {item?.attributes?.title || "Teaching pro course near me"}
            </HeadingText>
            {teachingProClubsNearMe?.length > 8 ? (
              <button
                onClick={() => {
                  setViewAll(!viewAll);
                }}
          ...
          <ul role="list" className={CoursesGrid}>
            {teachingProClubsNearMe
              ?.slice(
                0,
                viewAll === true ? LENGTH : teachingProClubsNearMe.length
              )
              .map((client, index) => {
                const ratings =
